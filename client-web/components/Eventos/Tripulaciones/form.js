...
const Form = forwardRef(({ save, isEdit, id, onClose, idEvent, onSelectConductor }, ref) => {
  const [datos, setDatos] = useState(null);
  const { data: categorias, mutate: catMutate, isLoading: catLoading } = CategoriaService.getData({
    page: 1,
    rowsPerPage: 30,
    // order_by: '-id',
    event_id: idEvent,
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (fileRejections) => {
      fileRejections.forEach((file) => {
        console.error(`Rejected file: ${file.file.name}, reason: ${file.errors[0].message}`);
      });
    },
  });
      ...
      <form ref={ref} onSubmit={onSave} className='flex flex-col gap-2'>
        <ModalBody>
          <SearchInput 
            ...
            initialValue={form.data.piloto?.nombre ? `${form.data.piloto.nombre} ${form.data.piloto.apellidos}` : ''}
          />
          ...
          <SearchInput 
            ...
            initialValue={form.data.navegante?.nombre ? `${form.data.navegante.nombre} ${form.data.navegante.apellidos}` : ''}
          />
          ...
          <p className='text-sm text-success'>
            {isEdit ? 'Cambiar Foto (Opcional)' : 'Foto del Vehiculo (Opcional)'}
          </p>
          ...
          <div className="py-2 px-4">
            ...
            >
              <input {...getInputProps()} />
              {isDragActive ? (
                <p className='text-sm text-center'>Suelta la imagen aquí...</p>
              ) : (
